    ...
    private findPython(): string {
        try {
            ...
            if (compare(pythonVersion, "3.8.0") < 0) {
                this.analytics.logCrash(EventType.ERROR_PYTHON_VERSION, new Error(`Python version too old: ${pythonVersion}`));
                this.outputChannel.appendLine(`Found Python in version ${pythonVersion}. Python >=3.8 must be installed.`);
            ...
            try {
                ...
                this.analytics.logCrash(EventType.ERROR_PYTHON_VERSION, new Error("Python not found"));
